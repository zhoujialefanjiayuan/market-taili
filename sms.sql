...
insert into ap_humancheck  select a.badloan, a.ap_id,a.is_human,a.ap_uid,b.application_id as ap_success,user_channel.channel,max(a.created_at) as created_at from ap_ishumancheck a left join bill.bill b on b.application_id=a.ap_id left join user_channel on user_channel.uid = a.ap_uid group by a.ap_uid ;
